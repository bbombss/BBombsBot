...
from src.models import BBombsBot, BBombsBotContext, BBombsBotPlugin
from src.static import FAIL_EMBED_COLOUR, FAIL_EMOJI

logger = logging.getLogger(__name__)

errorhandler = BBombsBotPlugin("errorhandler")
...
async def application_command_error_handler(event: lightbulb.CommandErrorEvent) -> None:
    assert isinstance(event.context, BBombsBotContext)
    ...
    await ctx.respond(
        embed=hikari.Embed(
            ...
            description="""An unhandled exception has occurred in the BBombsBot application.
